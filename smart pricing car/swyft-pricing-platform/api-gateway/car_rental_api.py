...
_PLATFORM_ROOT = _Path(__file__).resolve().parents[1]
for _d in (
    _PLATFORM_ROOT,
    _PLATFORM_ROOT / "services" / "pricing-engine",
    _PLATFORM_ROOT / "services" / "admin-service",
):
    _s = str(_d)
...
from middleware.fraud import FraudMiddleware
...
app.add_middleware(FraudMiddleware)
...
DATASET_PATH = Path(__file__).resolve().parent / "data" / "car_rental_sample_augmented_demo.csv"
...
class QuoteRequest(BaseModel):
    ...
    mileage: str = "200 miles per rental"
    group: str = "Economy"
...
@app.post("/auth/login")
def ui_auth_login(req: UiLoginRequest):
    """JWT for admin UI — secrets from env only."""
    import time

    import jwt

    secret = os.environ.get("ADMIN_UI_JWT_SECRET", "").strip()
    pwd = os.environ.get("ADMIN_UI_PASSWORD", "").strip()
    if not secret or not pwd or req.password != pwd:
        raise HTTPException(status_code=401, detail={"code": "unauthorized", "message": "Invalid credentials"})
    now = int(time.time())
    exp = now + int(os.environ.get("ADMIN_UI_JWT_TTL_SEC", "3600"))
    token = jwt.encode({"sub": req.username, "iat": now, "exp": exp}, secret, algorithm="HS256")
    return {"access_token": token, "expires_in": exp - now, "token_type": "Bearer"}
...
def ui_auth_refresh(
    ...
    try:
        payload = jwt.decode(raw, secret, algorithms=["HS256"], options={"verify_exp": False})
    except Exception as e:
        raise HTTPException(status_code=401, detail="invalid token") from e
    now = int(time.time())
    exp = now + int(os.environ.get("ADMIN_UI_JWT_TTL_SEC", "3600"))
    token = jwt.encode({"sub": payload.get("sub", "admin"), "iat": now, "exp": exp}, secret, algorithm="HS256")
    return {"access_token": token, "expires_in": exp - now, "token_type": "Bearer"}
